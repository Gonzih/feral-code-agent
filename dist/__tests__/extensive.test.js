...
describe('Extensive Tests', () => {
    ...
    describe.each(messageTypes)('MessageBus with type: %s', (type) => {
        it('should handle message type', () => {
            ...
            const message = { type, content: 'test content', from: 'sender', to: 'test' };
            bus.send(message);
    ...
    describe('Additional Coordinator Tests', () => {
        ...
        it.each(messageTypes)('should handle message type %s in bus', (type) => {
            const bus = new messageBus_1.MessageBus();
            const message = { type, content: 'test', from: 'test', to: 'test' };
        ...
        it.each(agentNames)('should have %s agent', (name) => {
            const llm = new mockLLM_1.MockLLM();
            const coordinator = new coordinator_1.Coordinator(llm);
            expect(coordinator).toBeInstanceOf(coordinator_1.Coordinator);
        });
    });
    ...
    describe('Message Variations', () => {
        it.each([
            ...
            const bus = new messageBus_1.MessageBus();
            const message = { ...msg, content: 'test' };
    ...
    describe('Coordinator Edge Cases', () => {
        it('should handle null task', async () => {
            const llm = new mockLLM_1.MockLLM();
            const coordinator = new coordinator_1.Coordinator(llm);
            const result = await coordinator.processTask(null);
            expect(result).toBe('Reviewed code');
        });
        it('should handle undefined task', async () => {
            const llm = new mockLLM_1.MockLLM();
            const coordinator = new coordinator_1.Coordinator(llm);
            const result = await coordinator.processTask(undefined);
            expect(result).toBe('Reviewed code');
        });
    ...
    describe('MessageBus Edge Cases', () => {
        it('should handle null message', () => {
            const bus = new messageBus_1.MessageBus();
            expect(() => bus.send(null)).not.toThrow();
        });
        it('should handle undefined message', () => {
            const bus = new messageBus_1.MessageBus();
            expect(() => bus.send(undefined)).not.toThrow();
        });
        it('should handle message without to', () => {
            const bus = new messageBus_1.MessageBus();
            const message = { type: 'test', content: 'test', from: 'test' };
            expect(() => bus.send(message)).not.toThrow();
        });
    });
    ...
    describe('Bulk Tests 2', () => {
        it.each(Array.from({ length: 20 }, (_, i) => `message${i}`))('should handle %s', (msg) => {
            const bus = new messageBus_1.MessageBus();
            const message = { type: 'test', content: msg, from: 'test', to: 'test' };
            expect(() => bus.send(message)).not.toThrow();
        });
    });
...
//# sourceMappingURL=extensive.test.js.map